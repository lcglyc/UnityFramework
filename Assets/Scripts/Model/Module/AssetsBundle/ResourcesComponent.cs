...
namespace ECSModel
{
    ...
    public class ResourcesComponent : Component
    {
        ...
        public override void Dispose()
        {
            ...
            {
                //TODO:这里要检查其他的Resources资源是否有未释放的情况
                if (abInfo.Value == null)
                {
                    continue;
                }
        ...
        /// <param name="assetBundleName"></param>
        /// <returns></returns>
        public async UniTask LoadBundleAsync(string assetBundleName)
        {
            assetBundleName = assetBundleName.ToLower();
            string[] dependencies = AssetBundleHelper.GetSortedDependencies(assetBundleName);
            //   Log.Debug($"-----------dep load {assetBundleName} dep: {dependencies.ToList().ListToString()}");
        ...
        public async UniTask LoadOneBundleAsync(string assetBundleName)
        {
            ABInfo abInfo;
            if (this.bundles.TryGetValue(assetBundleName, out abInfo))
            {

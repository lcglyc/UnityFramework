...
namespace ET
{
    ...
    {
        public static async UniTask<AssetBundle> UnityLoadBundleAsync(string path)
        {
            var tcs = UniTask<AssetBundle>.Create(true);
            AssetBundleCreateRequest request = AssetBundle.LoadFromFileAsync(path);
            request.completed += operation => { tcs.SetResult(request.assetBundle); };
            return await tcs;
        }

        public static async UniTask<UnityEngine.Object[]> UnityLoadAssetAsync(AssetBundle assetBundle)
        {
            var tcs = UniTask<UnityEngine.Object[]>.Create(true);
    ...
    public static class ResourcesComponentSystem
    {
        ...
        // 一帧卸载一个包，避免卡死
        public static async UniTask UnloadBundleAsync(this ResourcesComponent self, string assetBundleName, bool unload = true)
        {
        ...
        /// </summary>
        public static async UniTask LoadBundleAsync(this ResourcesComponent self, string assetBundleName)
        {
            assetBundleName = assetBundleName.BundleNameToLower();
            ...
            {
                async UniTask LoadDependency(string dependency, List<ABInfo> abInfosList)
                {
                    CoroutineLock coroutineLock = null;
                    try
                    {
                ...
                using (ListComponent<UniTask> tasks = ListComponent<UniTask>.Create())
                {
                    ...
                    await UniTaskHelper.WaitAll(tasks);
        ...
        private static async UniTask<ABInfo> LoadOneBundleAsync(this ResourcesComponent self, string assetBundleName)
        {
            assetBundleName = assetBundleName.BundleNameToLower();
            ABInfo abInfo;
            if (self.bundles.TryGetValue(assetBundleName, out abInfo))
        ...
        // 加载ab包中的all assets
        private static async UniTask LoadOneBundleAllAssets(this ResourcesComponent self, ABInfo abInfo)
        {
            CoroutineLock coroutineLock = null;
            try
            {
